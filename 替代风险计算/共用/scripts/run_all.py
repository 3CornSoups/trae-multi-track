...
# 跨目录导入：theme_pairs 属实验三专属脚本（实验三_要素凝练主题/scripts/），
# 运行本脚本时把该目录注入 sys.path（tests 由 共用/tests/conftest.py 统一注入）。
_THEME_SCRIPTS = os.path.join(
    os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
    '实验三_要素凝练主题', 'scripts')
if _THEME_SCRIPTS not in sys.path:
    sys.path.insert(0, _THEME_SCRIPTS)
